...
// Generated from commit: 14e58b8d6243bdb58804ba5c890bfcaafcc054b5
...
// NetworkIDs maps network names to their respective IDs
...
func GetNetworkID(network string) (int, error) {
	if id, exists := NetworkIDs[network]; exists {
		return id, nil
	}
	return 0, fmt.Errorf("unknown network: %s", network)
}

// GetNetworkName returns the network name for a given network ID
func GetNetworkName(id int) (string, error) {
	for name, nid := range NetworkIDs {
		if nid == id {
			return name, nil
		}
	}
